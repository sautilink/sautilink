...
const knownSections = new Set([...navigation.map(({ id }) => id), 'thread', 'safety', 'settings']);
...
function PrimaryRail({ section, member, navigationItems, messageUnreadCount, onNavigate, onCompose, onMore }) {
  return (
      ...
      <div className="primary-rail-inner">
        ...
        <nav className="desktop-navigation" aria-label="SautiLink">
          {navigationItems.map((item) => (
            <NavigationItem key={item.id} item={item.id === 'messages' ? { ...item, badge: messageUnreadCount } : item} active={section === item.id} onSelect={onNavigate} />
          ))}
...
function ComposerEntry({ member, onCompose }) {
  return (
    <section className="composer-entry" aria-label="Create Post">
...
function DiscoverScreen({ onPreviewAction }) {
  ...
  const visibleTrends = trends.filter((trend) => `${trend.label} ${trend.context}`.toLowerCase().includes(normalizedQuery));
  const visibleSuggestions = suggestions.filter((person) => `${person.name} ${person.handle}`.toLowerCase().includes(normalizedQuery));
  return (
    <>
      <ScreenHeader title="Discover" subtitle="Find useful voices, topics and conversations" />
      <section className="discover-search">
        <Search aria-hidden="true" />
        <input type="search" aria-label="Search SautiLink" placeholder="Search voices, posts and Sautify" value={query} onChange={(event) => setQuery(event.target.value)} />
        <kbd>/</kbd>
      </section>
      ...
      <section className="content-section voices-section">
        ...
        {visibleSuggestions.length ? visibleSuggestions.map((person) => <SuggestionRow key={person.handle} person={person} />) : <EmptyState icon={Search} title="No public voices found" copy="Try a different name or topic." />}
      </section>
    </>
  );
}

function MessagesScreen({ conversations, onSend, onMarkRead, onBlock, onReport, onDelete }) {
  const [query, setQuery] = useState('');
  ...
  const activeConversation = conversations.find(({ id }) => id === activeId) || null;
  ...
  const submitMessage = (event) => {
    ...
    if (!activeConversation || !text || activeConversation.blocked) return;
    onSend(activeConversation.id, text);
    setDraft('');
  };

  return (
    <>
      <ScreenHeader title="Messages" subtitle="Private one-to-one conversations" />
      <section className={`messages-layout${activeConversation ? ' has-active-conversation' : ''}`} aria-label="Direct messages">
        <aside className="message-inbox" aria-label="Conversation list">
          ...
          <div className="conversation-list">
            {visibleConversations.length ? visibleConversations.map((conversation) => (
              <button
                className={`conversation-row${activeId === conversation.id ? ' is-active' : ''}${conversation.unread ? ' is-unread' : ''}`}
                type="button"
                key={conversation.id}
                onClick={() => openConversation(conversation.id)}
                aria-current={activeId === conversation.id ? 'true' : undefined}
              >
                <Avatar initials={conversation.participant.initials} tone={conversation.participant.tone} size="small" />
                <span className="conversation-summary">
                  <span className="conversation-name"><strong>{conversation.participant.name}</strong><time>{conversation.updatedAt}</time></span>
                  <span className="conversation-preview">{conversation.blocked ? 'You blocked this account' : conversation.lastMessage}</span>
                </span>
                {conversation.unread ? <span className="conversation-unread" aria-label={`${conversation.unread} unread messages`}>{conversation.unread}</span> : null}
              </button>
            )) : <EmptyState icon={Search} title="No conversations found" copy="Try another name or message." />}
          </div>
        </aside>

        {activeConversation ? (
          <article className="message-thread" aria-label={`Conversation with ${activeConversation.participant.name}`}>
            <header className="message-thread-header">
              <button className="message-mobile-back" type="button" onClick={() => setActiveId(null)} aria-label="Back to conversations"><ChevronLeft aria-hidden="true" /></button>
              <Avatar initials={activeConversation.participant.initials} tone={activeConversation.participant.tone} size="small" />
              <span><strong>{activeConversation.participant.name}</strong><small>{activeConversation.participant.handle}</small></span>
              <div className="message-thread-actions">
                <button className="icon-button" type="button" onClick={() => setActionsOpen((value) => !value)} aria-expanded={actionsOpen} aria-label="Conversation safety actions"><MoreHorizontal aria-hidden="true" /></button>
                {actionsOpen ? <div className="message-action-menu" role="menu">
                  <button type="button" role="menuitem" onClick={() => { onBlock(activeConversation.id); setActionsOpen(false); }}>{activeConversation.blocked ? 'Unblock account' : 'Block account'}</button>
                  <button type="button" role="menuitem" disabled={activeConversation.reported} onClick={() => { onReport(activeConversation.id); setActionsOpen(false); }}><ShieldAlert aria-hidden="true" />{activeConversation.reported ? 'Reported' : 'Report conversation'}</button>
                  <button className="is-danger" type="button" role="menuitem" onClick={() => { onDelete(activeConversation.id); setActiveId(null); setActionsOpen(false); }}><Trash2 aria-hidden="true" />Delete conversation</button>
                </div> : null}
              </div>
            </header>
            ...
            <div className="message-history" aria-live="polite">
              ...
              {activeConversation.messages.map((message) => (
                <div className={`message-bubble-row is-${message.sender}`} key={message.id}>
                  <div className="message-bubble">
                    <p>{message.text}</p>
                    <span><time>{message.time}</time>{message.status ? ` · ${message.status}` : ''}</span>
                  </div>
                </div>
              ))}
              {activeConversation.blocked ? <div className="message-safety-note" role="status"><ShieldAlert aria-hidden="true" /><span>You blocked this account. Unblock it to send another message.</span></div> : null}
            </div>

            <form className="message-composer" onSubmit={submitMessage}>
              <label htmlFor="direct-message-draft" className="sr-only">Message {activeConversation.participant.name}</label>
              <textarea
                id="direct-message-draft"
                value={draft}
                onChange={(event) => setDraft(event.target.value.slice(0, 1000))}
                placeholder={activeConversation.blocked ? 'Unblock this account to reply' : 'Write a message'}
                rows="1"
                disabled={activeConversation.blocked}
              />
              <button type="submit" disabled={!draft.trim() || activeConversation.blocked} aria-label="Send message"><Send aria-hidden="true" /></button>
            </form>
          </article>
        ) : (
          <div className="message-thread-empty">
            ...
            <p>Select a conversation to read and reply. This preview stays on this device.</p>
          </div>
        )}
      </section>
    </>
...
function CirclesScreen({ joined, onToggleCircle, onPreviewAction }) {
  ...
  if (selectedCircle) {
    ...
    return (
      <>
        <ScreenHeader title={circle.name} subtitle={`${circle.access} Sautify · ${circle.members} members`}>
          <button className="icon-button" type="button" onClick={() => setSelectedCircle(null)} aria-label="Back to Sautify"><ChevronLeft aria-hidden="true" /></button>
          <button className={isJoined ? 'small-secondary is-joined' : 'small-primary'} type="button" onClick={() => onToggleCircle(circle.id)}>{isJoined ? 'Joined' : circle.access === 'Approval' ? 'Request to join' : 'Join Sautify'}</button>
        </ScreenHeader>
        ...
        <section className="circle-detail-grid">
          <article className="circle-detail-panel"><span>Sautify health</span><strong>{circle.active}</strong><p>Conversation quality, membership and moderator actions will be observable before launch.</p></article>
          <article className="circle-detail-panel"><span>Your access</span><strong>{isJoined ? circle.role || 'Member' : 'Not joined'}</strong><p>{isJoined ? 'You can read and participate under the visible Sautify rules.' : 'Review the rules before joining this Sautify.'}</p></article>
        </section>
  ...
  return (
    <>
      <ScreenHeader title="Sautify" subtitle="Focused communities with clear ownership">
        <button className="small-secondary" type="button" onClick={() => onPreviewAction('Create a Sautify')}><UsersRound aria-hidden="true" />Create</button>
      </ScreenHeader>
      <section className="circle-intro">
        ...
        <p>Sautify communities can be open, private or approval-based. Moderators and rules remain visible before you join.</p>
      </section>
      <div className="circle-filters" role="tablist" aria-label="Sautify filter">
        <button type="button" role="tab" aria-selected={filter === 'yours'} onClick={() => setFilter('yours')}>Your Sautify <span>{joined.size}</span></button>
      ...
      </div> : <EmptyState icon={UsersRound} title="Your Sautify list is clear." copy="Join a Sautify from Discover and it will stay organized here." />}
    </>
  );
}
  ...
  return (
    <>
      ...
      {savedPosts.length ? (
        ...
        <EmptyState icon={Bookmark} title="Keep the posts you want to return to." copy="Use the bookmark action on any post. Your Saved list is private." />
      )}
    </>
  );
}
...
function ProfileScreen({ member, viewMode, onViewModeChange, following, onFollow, liked, saved, onLike, onSave, onOpenMedia, onOpenThread, onEdit, onPreviewAction }) {
  ...
  return (
    <>
      ...
      {tab === 'replies' ? <EmptyState icon={MessageCircle} title="No replies to show yet." copy="Replies will appear here without mixing them into the member’s main post list." /> : null}
  ...
  return (
    <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="compose-dialog" role="dialog" aria-modal="true" aria-labelledby="compose-title">
        <header><button className="icon-button" type="button" onClick={onClose} aria-label="Close composer"><X aria-hidden="true" /></button><h2 id="compose-title">Create Post</h2><button className="draft-action" type="button" aria-expanded={draftsOpen} onClick={() => setDraftsOpen((value) => !value)}>Drafts{drafts.length ? ` (${drafts.length})` : ''}</button></header>
        {draftsOpen ? <section className="draft-panel" aria-label="Saved drafts"><div><strong>Drafts on this device</strong><button type="button" onClick={() => setDraftsOpen(false)}>Done</button></div>{drafts.length ? drafts.map((draft) => <button className="draft-row" type="button" key={draft.id} onClick={() => { setText(draft.text); setAudience(draft.audience); setReplyAccess(draft.replyAccess || 'Everyone'); setAttachments(draft.media || []); setDrafts((current) => current.filter(({ id }) => id !== draft.id)); setDraftsOpen(false); }}><span>{draft.text || 'Media post'}</span><small>{draft.audience}{draft.media?.length ? ` · ${draft.media.length} media` : ''}</small></button>) : <p>No drafts yet. Your unfinished post will stay private here.</p>}</section> : null}
        ...
        <div className="compose-scroll-region">
          <div className="compose-body"><Avatar initials={member.initials} tone="graphite" /><textarea ref={textareaRef} value={text} onChange={(event) => setText(event.target.value.slice(0, 500))} placeholder="What deserves to be heard?" aria-label="Your post" rows={attachments.length ? '3' : '6'} /></div>
        ...
        <div className="compose-settings">
          <label><Globe2 aria-hidden="true" /><span>Audience</span><select value={audience} onChange={(event) => setAudience(event.target.value)} aria-label="Post audience"><option>Public</option><option>Followers</option><option>East Africa Builders</option></select><ChevronDown aria-hidden="true" /></label>
        ...
        {offline ? <div className="compose-offline"><CloudOff aria-hidden="true" />Offline posts will be saved as drafts.</div> : null}
...
function MobileNavigation({ section, navigationItems, messageUnreadCount, onNavigate, onCompose }) {
  const items = navigationItems.filter(({ id }) => ['stream', 'discover', 'messages', 'notifications'].includes(id));
  return (
    <nav className="mobile-navigation" aria-label="Mobile navigation">
      {items.slice(0, 2).map((item) => <NavigationItem key={item.id} item={item} active={section === item.id} onSelect={onNavigate} compact />)}
      <button className="mobile-compose" type="button" onClick={onCompose} aria-label="Create Post"><PenLine aria-hidden="true" /></button>
      {items.slice(2).map((item) => <NavigationItem key={item.id} item={item.id === 'messages' ? { ...item, badge: messageUnreadCount } : item} active={section === item.id} onSelect={onNavigate} compact />)}
    </nav>
  );
}

function MobileMenu({ open, member, navigationItems, messageUnreadCount, onClose, onNavigate }) {
  if (!open) return null;
  return (
    <div className="mobile-menu-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <aside className="mobile-menu" role="dialog" aria-modal="true" aria-label="Account menu">
        ...
        <nav>{navigationItems.map((item) => <NavigationItem key={item.id} item={item.id === 'messages' ? { ...item, badge: messageUnreadCount } : item} active={false} onSelect={(id) => { onNavigate(id); onClose(); }} />)}</nav>
      </aside>
    </div>
  );
}
...
export default function App({
  ...
  enableMessagingPreview = false,
  enableSettingsPreview = false,
  ...
  const [messageConversations, setMessageConversations] = useState(() => directMessageConversations.map((conversation) => ({ ...conversation, messages: conversation.messages.map((message) => ({ ...message })) })));
  const [menuOpen, setMenuOpen] = useState(false);
  const [toast, setToast] = useState('');
  const toastTimer = useRef(0);
  const navigationItems = [
    ...(enableMessagingPreview ? navigation : navigation.filter(({ id }) => id !== 'messages')),
    ...(enableSettingsPreview ? [{ id: 'settings', label: 'Settings', icon: Settings }] : []),
  ];
  ...
  useEffect(() => {
    if (!enableMessagingPreview && section === 'messages') setSection('stream');
  }, [enableMessagingPreview, section]);

  useEffect(() => {
    if (!enableSettingsPreview && section === 'settings') setSection('stream');
  }, [enableSettingsPreview, section]);
  ...
  const toggleCircle = (id) => {
    ...
    showToast(willJoin ? 'Sautify added to your list.' : 'Sautify removed from your list.');
  };

  const messageUnreadCount = messageConversations.reduce((total, conversation) => total + conversation.unread, 0);

  const markConversationRead = (id) => {
    setMessageConversations((current) => current.map((conversation) => conversation.id === id && conversation.unread ? { ...conversation, unread: 0 } : conversation));
  };

  const sendDirectMessage = (id, text) => {
    const nextMessage = { id: `message-${Date.now()}`, sender: 'me', text, time: 'Now', status: 'Sent' };
    setMessageConversations((current) => current.map((conversation) => conversation.id === id ? {
      ...conversation,
      lastMessage: text,
      updatedAt: 'Now',
  ...
  const toggleConversationBlock = (id) => {
    const isBlocked = !messageConversations.find((conversation) => conversation.id === id)?.blocked;
    setMessageConversations((current) => current.map((conversation) => conversation.id === id ? { ...conversation, blocked: isBlocked } : conversation));
    showToast(isBlocked ? 'Account blocked in this preview.' : 'Account unblocked in this preview.');
  };

  const reportConversation = (id) => {
    setMessageConversations((current) => current.map((conversation) => conversation.id === id ? { ...conversation, reported: true } : conversation));
    showToast('Conversation added to the seeded safety review queue.');
  };

  const deleteConversation = (id) => {
    setMessageConversations((current) => current.filter((conversation) => conversation.id !== id));
    showToast('Conversation deleted from this device preview.');
  };
  ...
  const sectionContent = useMemo(() => {
    ...
    if (section === 'settings' && enableSettingsPreview && SettingsComponent) return <SettingsComponent member={member} onPreviewAction={showToast} />;
    ...
    if (section === 'messages' && enableMessagingPreview) return <MessagesScreen conversations={messageConversations} onSend={sendDirectMessage} onMarkRead={markConversationRead} onBlock={toggleConversationBlock} onReport={reportConversation} onDelete={deleteConversation} />;
  ...
  }, [section, member, feedPosts, streamStatus, enableStreamLab, enableMediaPreview, enableConversationPreview, enableMessagingPreview, enableSettingsPreview, enableSafetyPreview, SafetyComponent, SettingsComponent, activeThread, liked, saved, joinedCircles, profileView, followingPublicProfile, messageConversations]);
  ...
  const submitPreviewSauti = ({ text, audience, replyAccess, media = [] }) => {
    ...
    setSection('stream');
    showToast('Your post appears at the top of this preview. Nothing reached production.');
  };
  ...
  return (
    <>
      ...
      <div className="app-shell">
        <PrimaryRail section={section} member={member} navigationItems={navigationItems} messageUnreadCount={messageUnreadCount} onNavigate={setSection} onCompose={() => setComposeOpen(true)} onMore={() => enableSettingsPreview ? setSection('settings') : enableSafetyPreview ? setSection('safety') : showToast('Settings, moderation and account controls will live here.')} />
      ...
      <MobileNavigation section={section} navigationItems={navigationItems} messageUnreadCount={messageUnreadCount} onNavigate={setSection} onCompose={() => setComposeOpen(true)} />
      <MobileMenu open={menuOpen} member={member} navigationItems={navigationItems} messageUnreadCount={messageUnreadCount} onClose={() => setMenuOpen(false)} onNavigate={setSection} />
