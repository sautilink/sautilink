...
test('Phase 32 production artifact targets production Supabase and removes preview copy', async () => {
  ...
  assert.match(html, /app\.css\?v=20260924-pwa-notification-optin1/);
  ...
  assert.match(html, /app\.js\?v=20260924-pwa-notification-optin1/);
...
test('Phase 32 production Worker is path-scoped and keeps the account-entry root outside its route', async () => {
  ...
  assert.doesNotMatch(config, /"binding": "AI"/);
  assert.doesNotMatch(config, /"name": "POST_TRANSLATION_LIMITER"/);
...
test('Phase 32 production build and verifier are permanent repository gates', async () => {
  ...
  assert.doesNotMatch(pkg.scripts.check, /post-translation/);
  ...
  for (const marker of [
    ...
    'app.js?v=20260924-pwa-notification-optin1',
    'pwa.js?v=20260924-pwa-notification-optin1',
  ...
  assert.match(buildScript, /APP_JS_RELEASE = '20260924-pwa-notification-optin1'/);
  ...
  assert.match(serviceWorker, /sautilink-shell-v75/);
  assert.match(serviceWorker, /20260924-pwa-notification-optin1/);
});
