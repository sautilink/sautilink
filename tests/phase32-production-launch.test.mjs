...
test('Phase 32 production artifact targets production Supabase and removes preview copy', async () => {
  ...
  assert.match(html, /app\.css\?v=20260922-password-recovery-fix1/);
  ...
  assert.match(html, /app\.js\?v=20260922-password-recovery-fix1/);
...
test('Phase 32 production build and verifier are permanent repository gates', async () => {
  ...
  for (const marker of [
    ...
    'app.js?v=20260922-password-recovery-fix1',
  ...
  assert.match(buildScript, /APP_JS_RELEASE = '20260922-password-recovery-fix1'/);
  ...
  assert.match(serviceWorker, /sautilink-shell-v71/);
  assert.match(serviceWorker, /20260922-password-recovery-fix1/);
});
