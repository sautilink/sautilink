...
test('Phase 32 production artifact targets production Supabase and removes preview copy', async () => {
  ...
  assert.match(html, /app\.css\?v=20260921-comments-back2/);
  ...
  assert.match(html, /app\.js\?v=20260921-comments-back2/);
...
test('Phase 32 production build and verifier are permanent repository gates', async () => {
  ...
  for (const marker of [
    ...
    'app.js?v=20260921-comments-back2',
  ...
  assert.match(buildScript, /APP_JS_RELEASE = '20260921-comments-back2'/);
  ...
  assert.match(serviceWorker, /sautilink-shell-v66/);
  assert.match(serviceWorker, /20260921-comments-back2/);
});
