...
test('Phase 32 production artifact targets production Supabase and removes preview copy', async () => {
  ...
  assert.match(html, /app\.css\?v=20260921-messages-notifications1/);
  ...
  assert.match(html, /app\.js\?v=20260921-messages-notifications1/);
...
test('Phase 32 production build and verifier are permanent repository gates', async () => {
  ...
  for (const marker of [
    ...
    'app.js?v=20260921-messages-notifications1',
  ...
  assert.match(buildScript, /APP_JS_RELEASE = '20260921-messages-notifications1'/);
  ...
  assert.match(serviceWorker, /sautilink-shell-v67/);
  assert.match(serviceWorker, /20260921-messages-notifications1/);
});
